    ...
    try {
        ...
        if (String(resp.error) === 'false' && resp.accounts) {
            ...
            return arr.map(function (a: any) {
                return {
                    id: Number(a.id),
                    accountId: a.accountId ? Number(a.accountId) : undefined, // <—
                    name: a.name,
...
async function getSummaryRobusto(session: string, accountId: number): Promise<any[]> {
    let http = makeClient('https://www.myfxbook.com/api');

    try {
        const r = await http.get('/get-summary.json', { params: { session, id: accountId } });
        const d = r?.data ?? {};
        if (!d.error && Array.isArray(d.summary)) return d.summary;
        if (d.error) console.log(`get-summary (id=${accountId}) -> ${d.message || 'error'}`);
    } catch (e: any) {
        console.log('get-summary err1:', e.message);
    }

    try {
        const url = `/get-summary.json?session=${session}&id=${accountId}`;
        const r = await http.get(url, { headers: { Cookie: 'PHPSESSID=' + session } });
        const d = r?.data ?? {};
        if (!d.error && Array.isArray(d.summary)) return d.summary;
        if (d.error) console.log(`get-summary(cookie) (id=${accountId}) -> ${d.message || 'error'}`);
    } catch (e: any) {
        console.log('get-summary err2:', e.message);
    }
    ...
    try {
        ...
        const d = r?.data ?? {};
        if (!d.error && Array.isArray(d.summary)) return d.summary;
        if (d.error) console.log(`get-summary(alt host) (id=${accountId}) -> ${d.message || 'error'}`);
    } catch (e: any) {
        console.log('get-summary err3:', e.message);
    }
...
async function main() {
    ...
    try {
        ...
        await accountsCsv.writeRecords(
        ...
        for (const acc of selected) {
            ...
            // SUMMARY por símbolo
            try {
                const idForSummary = (acc as any).accountId ?? acc.id;
                console.log(`Summary usando id=${acc.id} / accountId=${(acc as any).accountId ?? 'n/a'}`);

                const summary = await getSummaryRobusto(session, idForSummary);
                if (summary.length > 0) {
                    const rows = summary.map((s: any) => ({
                        symbol: s.symbol ?? '',
                        longs_trades: s.longs?.trades ?? '',
                        longs_pips: s.longs?.pips ?? '',
                        longs_profit: s.longs?.profit ?? '',
                        shorts_trades: s.shorts?.trades ?? '',
                        shorts_pips: s.shorts?.pips ?? '',
                        shorts_profit: s.shorts?.profit ?? '',
                        totals_trades: s.totals?.trades ?? '',
                        totals_pips: s.totals?.pips ?? '',
                        totals_profit: s.totals?.profit ?? '',
                        wonPercent: s.wonPercent ?? '',
                        lostPercent: s.lostPercent ?? '',
                    }));

                    const sCsv = createCsvWriter({
                        path: path.join(outDir, `summary_${acc.id}.csv`),
                        header: Object.keys(rows[0]).map((k) => ({ id: k, title: k })),
                    });
                    await sCsv.writeRecords(rows);
                    console.log(`summary_${acc.id}.csv generado (${rows.length} símbolos)`);
                } else {
                    console.log(`summary_${acc.id}: sin datos (probado id=${acc.id} / accountId=${(acc as any).accountId ?? 'n/a'})`);
                }
            } catch (e: any) {
                console.log(`summary_${acc.id}: no disponible (${e?.message || 'error'})`);
            }
        }
    } finally {
        try {
